				...
				case 'register' :
					...
					if ( $http_post ) {
						...
						$user_email = $_POST['user_email'];
                                                ...
                                                $user_weddingdate = $_POST['user_weddingdate'];

						$this->errors = self::register_new_user( $user_name, $user_email, $user_password, $user_retype_password,$user_weddingdate );
						if ( ! is_wp_error( $this->errors ) ) {
...
<?php
	 ...
	 */
	public static function register_new_user(  $user_name, $user_email, $user_password, $user_retype_password,$user_weddingdate  ) {
		$errors = new WP_Error();

		$sanitized_user_login = sanitize_user( $user_name );
                $user_name = apply_filters('user_registration_name', $user_name);
		$user_email = apply_filters( 'user_registration_email', $user_email );
                $user_password = apply_filters( 'user_registration_password', $user_password );
                $user_weddingdate = apply_filters('user_registration_weddingdate',$user_weddingdate);
		...
		/*if ( $sanitized_user_login == '' ) {
			...
			$errors->add( 'username_exists', __( '<strong>ERROR</strong>: This username is already registered, please choose another one.' ) );
                ...
                // Check the user name
		if ( '' == $user_name ) {
			$errors->add( 'empty_user_name', __( '<strong>ERROR</strong>: Please enter user name' ) );
		}

                // Check the password
		if ( '' == $user_password ) {
			$errors->add( 'empty_user_password', __( '<strong>ERROR</strong>: Please enter password' ) );
		}elseif($user_password != $user_retype_password){
                       $errors->add( 'mismatch_user_password', __( '<strong>ERROR</strong>: Password doesnt match' ) );
                }
		...
		do_action( 'register_post', $user_name, $user_email, $user_weddingdate, $errors );
		...
		$user_pass = apply_filters( 'tml_user_registration_pass', md5($user_password) );
		...
		update_user_option( $user_id, 'default_password_nag', true, true ); //Set up the Password change nag.
                ...
                update_user_meta($user_id, 'weddingdate', $user_weddingdate);
